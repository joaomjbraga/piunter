...
type IdesCacheModule struct {
	BaseModule
	paths []string
}

func NewIdesCacheModule() *IdesCacheModule {
	return &IdesCacheModule{
		BaseModule: BaseModule{
			id:          "ides-cache",
			name:        "Cache de IDEs",
			description: "Limpa temporários de IDEs e resíduos de desinstalação",
		},
		paths: []string{
			".cache/Code - Insiders",
			".cache/Code",
			".config/Code - Insiders/Crashpad",
...
func (m *IdesCacheModule) IsAvailable() bool {
	return true
}

func (m *IdesCacheModule) Analyze(threshold int) (*types.AnalysisResult, error) {
	...
	for _, relative := range m.paths {
		...
		result.Items = append(result.Items, types.CleanableItem{
			...
			Size:        size,
			Type:        "directory",
			Description: fmt.Sprintf("Cache/Resíduo de IDE: %s", filepath.Base(path)),
		})
...
func (m *IdesCacheModule) Clean(dryRun bool) (*types.CleaningResult, error) {
	...
	if dryRun {
		...
		utils.Info(fmt.Sprintf("[DRY-RUN] Limparia %d artefatos de IDEs", len(analysis.Items)))
		return result, nil
	}
	...
	if result.ItemsRemoved > 0 {
		utils.Item(m.Name(), fmt.Sprintf("%d artefatos removidos", result.ItemsRemoved))
	}
