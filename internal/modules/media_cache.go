...
type MediaCacheModule struct {
	BaseModule
	paths []string
}

func NewMediaCacheModule() *MediaCacheModule {
	return &MediaCacheModule{
		BaseModule: BaseModule{
			id:          "media-cache",
			name:        "Cache de Mídia",
			description: "Limpa caches de mídia e aplicativos gráficos",
		},
		paths: []string{
			".cache/spotify",
			".cache/discord",
...
func (m *MediaCacheModule) IsAvailable() bool {
	return true
}

func (m *MediaCacheModule) Analyze(threshold int) (*types.AnalysisResult, error) {
	...
	for _, relative := range m.paths {
		...
		result.Items = append(result.Items, types.CleanableItem{
			...
			Size:        size,
			Type:        "directory",
			Description: fmt.Sprintf("Cache de mídia: %s", filepath.Base(path)),
		})
...
func (m *MediaCacheModule) Clean(dryRun bool) (*types.CleaningResult, error) {
	...
	if dryRun {
		...
		utils.Info(fmt.Sprintf("[DRY-RUN] Limparia %d caches de mídia", len(analysis.Items)))
		return result, nil
	}
