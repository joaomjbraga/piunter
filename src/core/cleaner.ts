...
import type { Report, CleanOptions } from '../types/index.js';
...
export class Cleaner {
  ...
  async clean(): Promise<Report> {
    ...
    const results = await Promise.all(
      availableModules.map(m =>
        m.clean(this.dryRun).catch(error => ({
          module: m.id,
          success: false,
          spaceFreed: 0,
          itemsRemoved: 0,
          errors: [`${m.name}: ${(error as Error).message}`],
        }))
      )
    );
  ...
  printReport(report: Report): void {
    ...
    const durationStr =
      duration < 60000 ? `${(duration / 1000).toFixed(1)}s` : `${(duration / 60000).toFixed(1)}min`;
    ...
    console.log(
      `    ${chalk.dim('-')} ${chalk.white('Espaco liberado:')} ${chalk.green(totalSize)}`
    );
    console.log(
      `    ${chalk.dim('-')} ${chalk.white('Itens removidos:')} ${chalk.cyan(totalItems)}`
    );
    console.log(
      `    ${chalk.dim('-')} ${chalk.white('Erros:')} ${totalErrors === '0' ? chalk.green(totalErrors) : chalk.red(totalErrors)}`
    );
